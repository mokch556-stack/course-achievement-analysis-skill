...
def check_sheet_structure(ws):
    ...
    # 检查毕业要求/指标点行
    found_indicator = any('指标点' in row_texts[r] or '毕业要求' in row_texts[r] for r in range(1, 15))
    if not found_indicator:
        errors.append("❌ 指标点行缺失或内容不完整")
    ...
    found_zhefen = any('折合分' in row_texts[r] for r in range(1, 15))
    if not found_full and not found_zhefen:
        errors.append("❌ 折合满分信息行缺失")

    # 列数检查：v3通用格式，列数=2(header) + sum(子项数) + 目标数，≥8列
    if max_col < 8:
        errors.append(f"❌ 列数过少（{max_col}列），应≥8列")
...
def check_data_consistency(ws, chengji_path=None):
    ...
    # 检查折合分是否合理（折合总分上限 = 各目标折合满分之和，通常 ≤300）
    zhehe_values = []
    ...
    if zhehe_values:
        # 检查是否所有值都在 0-300 之间
        out_of_range = [v for v in zhehe_values if v < 0 or v > 300]
        if out_of_range:
            errors.append(f"❌ 有{len(out_of_range)}个学生的折合总分超出0-300范围")
        # 检查是否有负值
