...
OUT_PATH = r"E:\qwenpaw\workspaces\default\附件7_农产品供应链_教学目标达成情况分析报告_v6_4.docx"
...
trs = tbl1.findall(f'{{{NS_W}}}tr')

# 保存分析/签字/空行（R7~R9，保留模板段落格式）
analysis_tr = trs[7]
sig_tr = trs[8]
blank_tr = trs[9]
tbl1.remove(analysis_tr)
tbl1.remove(sig_tr)
tbl1.remove(blank_tr)

# 删除旧数据行（R1~R6）
for tr in trs[1:7]:
    tbl1.remove(tr)

# 添加11个新行（5目标1 + 5目标2 + 1汇总）
for i in range(11):
    t1.add_row()

# 重新追加分析/签字/空行（保留模板原始格式）
tbl1.append(analysis_tr)
tbl1.append(sig_tr)
tbl1.append(blank_tr)
# 此时15行：R0表头 R1~R11新数据 R12分析 R13签字 R14空

# ===== 合并单元格 =====
# C0: 侧边标签跨R1-R11纵向合并
t1.rows[1].cells[0].merge(t1.rows[11].cells[0])
# C1: 毕业要求 - 目标1(R1-R5) + 目标2(R6-R10) 分开合并
t1.rows[1].cells[1].merge(t1.rows[5].cells[1])
t1.rows[6].cells[1].merge(t1.rows[10].cells[1])
# C2: 指标点
t1.rows[1].cells[2].merge(t1.rows[5].cells[2])
t1.rows[6].cells[2].merge(t1.rows[10].cells[2])
# C3-C4: 课程教学目标（横向+纵向合并）
t1.rows[1].cells[3].merge(t1.rows[5].cells[4])
t1.rows[6].cells[3].merge(t1.rows[10].cells[4])
# C6-C7: 分值(A) 每行单独合并
for ri in range(1, 11):
    t1.rows[ri].cells[6].merge(t1.rows[ri].cells[7])
# 汇总行：C1-C4合并，C5-C6合并
t1.rows[11].cells[1].merge(t1.rows[11].cells[4])
t1.rows[11].cells[5].merge(t1.rows[11].cells[6])

# ===== 计算各子项平均分和达成度 =====
g1_sub_names = ["阶段测试", "课堂笔记", "项目方案", "课程竞赛", "期末项目"]
g2_sub_names = ["阶段测试", "课堂笔记", "项目方案", "课程竞赛", "期末项目"]
# 各子项满分值（从教学大纲课点归属表推导）
# 平时25分 = 阶段测试7.5 + 课堂笔记2.5 + 项目方案10 + 课程竞赛5
G1_SUB_FULLS = [7.5, 2.5, 10, 5, 20]  # 目标1: 4平时+1期末
G2_SUB_FULLS = [7.5, 2.5, 10, 5, 30]  # 目标2: 4平时+1期末

g1_sub_avgs = [round(xlsx_totals[i] / xlsx_cnt, 2) if xlsx_cnt else 0 for i in range(4)]
g1_qm_avg = round(xlsx_totals[4] / xlsx_cnt, 2) if xlsx_cnt else 0
g2_sub_avgs = [round(xlsx_totals[i] / xlsx_cnt, 2) if xlsx_cnt else 0 for i in range(5, 9)]
g2_qm_avg = round(xlsx_totals[9] / xlsx_cnt, 2) if xlsx_cnt else 0
# 子项达成度
g1_sub_ds = [round(g1_sub_avgs[i] / G1_SUB_FULLS[i], 2) for i in range(4)]
g1_sub_ds.append(round(g1_qm_avg / 20, 2))
g2_sub_ds = [round(g2_sub_avgs[i] / G2_SUB_FULLS[i], 2) for i in range(4)]
g2_sub_ds.append(round(g2_qm_avg / 30, 2))

# ===== 填写数据 =====
# 目标1各子行（R1~R5）
t1.rows[1].cells[1].text = "毕业要求1"
t1.rows[1].cells[2].text = "1-1"
t1.rows[1].cells[3].text = "课程教学目标1"
for i in range(5):
    ri = 1 + i
    t1.rows[ri].cells[5].text = g1_sub_names[i]
    t1.rows[ri].cells[6].text = f"{G1_SUB_FULLS[i]}"
    t1.rows[ri].cells[8].text = f"{g1_sub_avgs[i] if i < 4 else g1_qm_avg}"
    t1.rows[ri].cells[9].text = f"{g1_sub_ds[i]:.2f}"

# 目标2各子行（R6~R10）
t1.rows[6].cells[1].text = "毕业要求1"
t1.rows[6].cells[2].text = "1-2"
t1.rows[6].cells[3].text = "课程教学目标2"
for i in range(5):
    ri = 6 + i
    t1.rows[ri].cells[5].text = g2_sub_names[i]
    t1.rows[ri].cells[6].text = f"{G2_SUB_FULLS[i]}"
    t1.rows[ri].cells[8].text = f"{g2_sub_avgs[i] if i < 4 else g2_qm_avg}"
    t1.rows[ri].cells[9].text = f"{g2_sub_ds[i]:.2f}"

# 汇总行（R11）
t1.rows[11].cells[1].text = "课程达成度"
t1.rows[11].cells[5].text = "课程达成度"
t1.rows[11].cells[9].text = f"{cv:.2f}"
print("[OK] Table1 filled")
# ======== 分析文本 ========
print("--- 生成分析文本 ---")
ac = t1.rows[12].cells[1]
